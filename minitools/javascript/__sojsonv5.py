...
__all__ = "get_anti_spider_sojsonv5",


def get_anti_spider_sojsonv5(js_string):
    script_content = r"""
var get_redirect = function(argv1, argv2){
...
"""
    ...
    re_handler = re.search(
        'dynamicurl\|(.*?)\|.*?wzwsquestion\|(.*?)\|.*?wzwsfactor\|(.*?)\|', js_string
    ) or re.search(
        'dynamicurl="(.*?)".*?wzwsquestion="(.*?)".*?wzwsfactor="(.*?)"', js_string)
    dynamicUrl, wzWsQuestion, wzWsFactor = re_handler.groups()
